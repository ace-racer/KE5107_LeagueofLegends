...
# train the model
tunegrid <- expand.grid(.size = c(5, 6, 8, 10, 20, 30), .decay=c(0.1))

model <- caret::train(bResult~., data=training_data, 
...
prediction_validate <- predict(model, validate_data)

# confusion matrix based on validation data
confusionMatrix(prediction_validate, validate_data$bResult)
