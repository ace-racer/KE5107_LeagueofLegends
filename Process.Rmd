...
getFirstNumber <- function(value, splitAtCharacter){
  if(value != "[]"){
    splits <- strsplit(value, splitAtCharacter)
...
for(i in 1: length(first_kill_elements))
{
  ...
  # Get the split character based on the column whose data is required
  splitCharacterForColumn = ','
  
  if(column_name == "Barons")
  {
    splitCharacterForColumn = ']'
  }
  
  matches_data[,blue_output_column_name] <- unlist(lapply(as.character(matches_data[,blue_column_name]), getFirstNumber, splitAtCharacter = splitCharacterForColumn))
  
  matches_data[,red_output_column_name] <- unlist(lapply(as.character(matches_data[,red_column_name]), getFirstNumber, splitAtCharacter = splitCharacterForColumn))
