...
for (season_num in 1:nrow(seasons)) {
  ...
  #match_season <- subset(matches, (matches$YearSeason==Season)) # extract only matches for this season
  print(Season)
  for (row  in 1:nrow(combinedPlayed)) { 
    ...
    matches$isBluePreferredLineup <- ifelse((matches$blueTop==btop & matches$blueADC==badc & matches$blueJungle==bjungle & matches$blueSupport==bsupport & matches$blueMiddle==bmiddle & matches$YearSeason==Season), 1, matches$isBluePreferredLineup)
    matches$isRedPreferredLineup <- ifelse((matches$redTop==rtop & matches$redADC==radc & matches$redJungle==rjungle & matches$redSupport==rsupport & matches$redMiddle==rmiddle  & matches$YearSeason==Season), 1, matches$isRedPreferredLineup)
    
    # In all the blueWins for this team, get the most popular champion for each pos
    ...
    # In all the redWins for this team, get the most popular champion for each pos
    ...
    matches$isBluePreferredChamps <- ifelse((matches$blueTopChamp==btopChamp & matches$blueADCChamp==badcChamp & matches$blueJungleChamp==bjungleChamp & matches$blueSupportChamp==bsupportChamp & matches$blueMiddleChamp==bmiddleChamp  & matches$YearSeason==Season), 1, matches$isBluePreferredChamps)
    matches$isRedPreferredChamps <- ifelse((matches$redTopChamp==rtopChamp & matches$redADCChamp==radcChamp & matches$redJungleChamp==rjungleChamp & matches$redSupportChamp==rsupportChamp & matches$redMiddleChamp==rmiddleChamp & matches$YearSeason==Season), 1, matches$isRedPreferredChamps)
  }
}
